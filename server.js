...
if (!process.env.STORAGE_HMAC_KEY) console.warn('WARNING: set STORAGE_HMAC_KEY for a stable deployment; generated key is for development only.');
...
async function handler(req, res) {
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/files/init') {
      ...
      if (!Number.isSafeInteger(input.size) || input.size < 0 || input.size > 2 ** 53 - 1) return fail(res, 400, 'invalid size');
      ...
      const totalChunks = Math.max(1, Math.ceil(input.size / input.chunkSize));
      await fs.mkdir(fileDir(id), { recursive: true, mode: 0o700 });
      await writeManifest(id, { id, filename: path.basename(input.filename), size: input.size, chunkSize: input.chunkSize, totalChunks, salt: input.salt, status: 'uploading', createdAt: new Date().toISOString() });
      return json(res, 201, { id, totalChunks, chunkSize: input.chunkSize });
    }
    const match = url.pathname.match(/^\/api\/files\/([a-f0-9]{32})(?:\/chunks\/(\d+))?$/);
    if (!match || !idOk(match[1])) return fail(res, 404, 'not found');
    const id = match[1];
    let manifest; try { manifest = await readManifest(id); } catch { return fail(res, 404, 'file not found'); }
    if (req.method === 'GET' && !match[2]) return json(res, 200, { ...manifest, present: await Promise.all(Array.from({ length: manifest.totalChunks }, async (_, i) => { try { await fs.access(chunkPath(id, i)); return i; } catch { return null; } })).then(a => a.filter(i => i !== null)) });
    if (!match[2] || !safeIndex(match[2])) return fail(res, 400, 'invalid chunk index');
    const index = Number(match[2]);
    if (index >= manifest.totalChunks) return fail(res, 416, 'chunk index out of range');
    if (req.method === 'PUT') {
      ...
      const mac = computeMac(id, index, body);
      await fs.writeFile(chunkPath(id, index), body, { mode: 0o600 });
      await fs.writeFile(macPath(id, index), mac, { mode: 0o600 });
      return json(res, 200, { id, index, bytes: body.length, sha256: crypto.createHash('sha256').update(body).digest('hex') });
    }
    if (req.method === 'GET') {
      const body = await fs.readFile(chunkPath(id, index));
      const storedMac = (await fs.readFile(macPath(id, index), 'utf8')).trim();
      if (!crypto.timingSafeEqual(Buffer.from(storedMac), Buffer.from(computeMac(id, index, body)))) return fail(res, 500, 'stored chunk integrity check failed');
      res.writeHead(200, { 'content-type': 'application/octet-stream', 'content-length': body.length, 'cache-control': 'no-store', 'x-chunk-sha256': crypto.createHash('sha256').update(body).digest('hex') }); return res.end(body);
    }
    if (req.method === 'POST' && url.pathname.endsWith('/complete')) { /* unreachable; retained for route clarity */ }
...
const serverHandler = async (req, res) => { if (!(await completeHandler(req, res))) await handler(req, res); };
